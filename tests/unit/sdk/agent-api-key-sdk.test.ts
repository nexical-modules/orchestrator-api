...
import { AgentApiKeySDK } from '../../../src/sdk/agent-api-key-sdk';

describe('AgentApiKeySDK', () => {
  let sdk: AgentApiKeySDK;
  ...
  beforeEach(() => {
    ...
    sdk = new AgentApiKeySDK(mockClient as unknown as Record<string, unknown>);
  });
  ...
  it('should call POST /keys on createAgentApiKey()', async () => {
  ...
  it('should handle failure on createAgentApiKey()', async () => {
  ...
  it('should call DELETE /keys/[id] on deleteAgentApiKey()', async () => {
    mockClient.request.mockResolvedValue({ success: true, data: {} });

    const args: unknown[] = [];
    const pathParams = '/keys/[id]'.match(/\[(\w+)\]/g) || [];
    pathParams.forEach(() => args.push('test-id'));
    ...
    await (
      sdk as unknown as Record<string, (...args: unknown[]) => Promise<unknown>>
    ).deleteAgentApiKey(...args);

    expect(mockClient.request).toHaveBeenCalled();
    const [callVerb, callPath] = mockClient.request.mock.calls[0];
    expect(callVerb).toBe('DELETE');

    let expectedPath = 'keys/[id]';
    pathParams.forEach((p) => {
      expectedPath = expectedPath.replace(p, 'test-id');
    });
    expect(callPath).toContain(expectedPath);
  ...
  it('should handle failure on deleteAgentApiKey()', async () => {
    mockClient.request.mockResolvedValue({ success: false, error: 'API Error' });
  ...
  it('should call GET /agent-api-key on list()', async () => {
    mockClient.request.mockResolvedValue({ success: true, data: {} });

    const args: unknown[] = [];
    const pathParams = '/agent-api-key'.match(/\[(\w+)\]/g) || [];
    pathParams.forEach(() => args.push('test-id'));
    ...
    let expectedPath = 'agent-api-key';
    pathParams.forEach((p) => {
      expectedPath = expectedPath.replace(p, 'test-id');
    });
    expect(callPath).toContain(expectedPath);
  ...
  it('should call GET /agent-api-key/[id] on get()', async () => {
    mockClient.request.mockResolvedValue({ success: true, data: {} });

    const args: unknown[] = [];
    const pathParams = '/agent-api-key/[id]'.match(/\[(\w+)\]/g) || [];
    pathParams.forEach(() => args.push('test-id'));
    ...
    let expectedPath = 'agent-api-key/[id]';
    pathParams.forEach((p) => {
      expectedPath = expectedPath.replace(p, 'test-id');
    });
    expect(callPath).toContain(expectedPath);
