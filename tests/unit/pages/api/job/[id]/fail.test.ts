...
import { POST } from '../../../../../../src/pages/api/job/[id]/fail';
vi.mock('../../../../../../src/actions/fail-job');
vi.mock('@/lib/api/api-guard');

describe('Job API - POST ../../../../../../src/pages/api/job/[id]/fail', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call FailJobAction and return success', async () => {
    const query = ['GET', 'DELETE'].includes('POST'.toUpperCase())
      ? `?error=${encodeURIComponent(String({}))}`
      : '';
    ...
    vi.mocked(FailJobAction.run).mockResolvedValue({
      success: true,
      data: { id: 'test-id' },
