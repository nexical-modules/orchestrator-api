...
import { POST } from '../../../../../../src/pages/api/job/[id]/progress';
vi.mock('../../../../../../src/actions/update-progress-job');
vi.mock('@/lib/api/api-guard');

describe('Job API - POST ../../../../../../src/pages/api/job/[id]/progress', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call UpdateProgressJobAction and return success', async () => {
    const query = ['GET', 'DELETE'].includes('POST'.toUpperCase())
      ? `?progress=${encodeURIComponent(String(100))}`
      : '';
