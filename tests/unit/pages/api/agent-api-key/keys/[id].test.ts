...
import { DELETE } from '../../../../../../src/pages/api/agent-api-key/keys/[id]';
vi.mock('../../../../../../src/actions/delete-agent-api-key');
vi.mock('@/lib/api/api-guard');

describe('AgentApiKey API - DELETE ../../../../../../src/pages/api/agent-api-key/keys/[id]', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(ApiGuard.protect).mockResolvedValue(undefined);
  });

  it('should call DeleteAgentApiKeyAction and return success', async () => {
    ...
    vi.mocked(DeleteAgentApiKeyAction.run).mockResolvedValue({
      success: true,
      data: { id: 'test-id' },
