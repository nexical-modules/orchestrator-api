...
vi.mock('@/lib/core/db', () => {
  ...
  (globalThis as unknown as Record<string, () => void>)._resetAgentApiKeyServiceMocks =
    resetImplementations;
  return {
...
describe('AgentApiKeyService', () => {
  beforeEach(() => {
    ...
    if (globalAny._resetAgentApiKeyServiceMocks) {
      globalAny._resetAgentApiKeyServiceMocks();
    }
  ...
  describe('list', () => {
    it('should return a list of AgentApiKeys', async () => {
      const mockData = [{ id: '1' }];
      vi.mocked(db.agentApiKey.findMany).mockResolvedValue(
      ...
      expect(db.agentApiKey.findMany).toHaveBeenCalled();
    });
  ...
  describe('get', () => {
    it('should return a single AgentApiKey', async () => {
      const mockData = { id: '1' };
      vi.mocked(db.agentApiKey.findUnique).mockResolvedValue(
        mockData as unknown as Record<string, unknown>,
      );
      ...
      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockData);
      expect(db.agentApiKey.findUnique).toHaveBeenCalledWith(
    ...
    it('should handle not found', async () => {
      vi.mocked(db.agentApiKey.findUnique).mockResolvedValue(null);

      const result = await AgentApiKeyService.get('1');

      expect(result.success).toBe(false);
      expect(result.error).toBe('agentApiKey.service.error.not_found');
    });

    it('should handle errors when getting', async () => {
      vi.mocked(db.agentApiKey.findUnique).mockRejectedValue(new Error('DB Error'));

      const result = await AgentApiKeyService.get('1');

      expect(result.success).toBe(false);
      expect(result.error).toBe('agentApiKey.service.error.get_failed');
    });
  });

  describe('create', () => {
    it('should create a new AgentApiKey', async () => {
      const mockData = { id: '1', name: 'test' };
      vi.mocked(db.agentApiKey.create).mockResolvedValue(
        mockData as unknown as Record<string, unknown>,
      );
      ...
      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockData);
      expect(db.agentApiKey.create).toHaveBeenCalled();
    });

    it('should handle errors when creating', async () => {
      vi.mocked(db.agentApiKey.create).mockRejectedValue(new Error('DB Error'));

      const result = await AgentApiKeyService.create({} as Record<string, unknown>);

      expect(result.success).toBe(false);
      expect(result.error).toBe('agentApiKey.service.error.create_failed');
    });
  });

  describe('update', () => {
    it('should update an existing AgentApiKey', async () => {
      const mockData = { id: '1', name: 'updated' };
      vi.mocked(db.agentApiKey.update).mockResolvedValue(
        mockData as unknown as Record<string, unknown>,
      );
      ...
      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockData);
      expect(db.agentApiKey.update).toHaveBeenCalled();
    });

    it('should handle errors when updating', async () => {
      vi.mocked(db.agentApiKey.update).mockRejectedValue(new Error('DB Error'));

      const result = await AgentApiKeyService.update('1', {} as Record<string, unknown>);

      expect(result.success).toBe(false);
      expect(result.error).toBe('agentApiKey.service.error.update_failed');
    });
  });

  describe('delete', () => {
    it('should delete an AgentApiKey', async () => {
      vi.mocked(db.agentApiKey.delete).mockResolvedValue({} as unknown as Record<string, unknown>);

      const result = await AgentApiKeyService.delete('1');

      expect(result.success).toBe(true);
      expect(db.agentApiKey.delete).toHaveBeenCalled();
    });

    it('should handle errors when deleting', async () => {
      vi.mocked(db.agentApiKey.delete).mockRejectedValue(new Error('DB Error'));

      const result = await AgentApiKeyService.delete('1');

      expect(result.success).toBe(false);
      expect(result.error).toBe('agentApiKey.service.error.delete_failed');
    });
  });
});
