...
vi.mock('@/lib/core/db', () => {
  ...
  (globalThis as unknown as Record<string, () => void>)._resetAgentServiceMocks =
    resetImplementations;
  return {
...
describe('AgentService', () => {
  beforeEach(() => {
    ...
    if (globalAny._resetAgentServiceMocks) {
      globalAny._resetAgentServiceMocks();
    }
  ...
  describe('get', () => {
    it('should return a single Agent', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data).toEqual(mockData);
      expect(db.agent.findUnique).toHaveBeenCalledWith(
    ...
    it('should handle errors when getting', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.error).toBe('agent.service.error.get_failed');
    });
  });
  ...
  describe('create', () => {
    ...
    it('should handle errors when creating', async () => {
      ...
      const result = await AgentService.create({} as Record<string, unknown>);

      expect(result.success).toBe(false);
      expect(result.error).toBe('agent.service.error.create_failed');
    });
  });
  ...
  describe('update', () => {
    ...
    it('should handle errors when updating', async () => {
      ...
      const result = await AgentService.update('1', {} as Record<string, unknown>);

      expect(result.success).toBe(false);
      expect(result.error).toBe('agent.service.error.update_failed');
    });
  });

  describe('delete', () => {
    it('should delete an Agent', async () => {
      vi.mocked(db.agent.delete).mockResolvedValue({} as unknown as Record<string, unknown>);

      const result = await AgentService.delete('1');

      expect(result.success).toBe(true);
      expect(db.agent.delete).toHaveBeenCalled();
    });

    it('should handle errors when deleting', async () => {
      vi.mocked(db.agent.delete).mockRejectedValue(new Error('DB Error'));

      const result = await AgentService.delete('1');

      expect(result.success).toBe(false);
      expect(result.error).toBe('agent.service.error.delete_failed');
    });
  });
});
