...
import { OrchestrationService } from '../../../src/actions/../services/orchestration-service';
...
vi.mock('../../../src/actions/../services/orchestration-service');
...
describe('CancelJobAction', () => {
  ...
  it('should be defined', () => {
    expect(CancelJobAction).toBeDefined();
  });

  it('should run (smoke test)', async () => {
    const orchestrationServiceMock = vi.mocked(OrchestrationService);
    commonServiceMethods.forEach((m) => {
      if ((orchestrationServiceMock as unknown as Record<string, vi.Mock>)[m])
        (orchestrationServiceMock as unknown as Record<string, vi.Mock>)[m].mockResolvedValue({
          success: true,
          data: { id: '1' },
        });
    });
    ...
    const result = await CancelJobAction.run(
      {
        id: '1',
        userId: 'user-1',
        teamId: 'team-1',
  ...
  it('should call OrchestrationService on success', async () => {
    const mockData = { id: '1' };
  ...
  it('should return error when OrchestrationService fails', async () => {
    const serviceMock = vi.mocked(OrchestrationService);
    commonServiceMethods.forEach((m) => {
