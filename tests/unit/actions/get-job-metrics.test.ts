...
import { JobMetricsService } from '../../../src/actions/../services/job-metrics-service';
...
vi.mock('../../../src/actions/../services/job-metrics-service');
...
describe('GetJobMetricsAction', () => {
  ...
  it('should be defined', () => {
    expect(GetJobMetricsAction).toBeDefined();
  });

  it('should run (smoke test)', async () => {
    const jobMetricsServiceMock = vi.mocked(JobMetricsService);
    commonServiceMethods.forEach((m) => {
      if ((jobMetricsServiceMock as unknown as Record<string, vi.Mock>)[m])
        (jobMetricsServiceMock as unknown as Record<string, vi.Mock>)[m].mockResolvedValue({
          success: true,
          data: { id: '1' },
        });
    });
    ...
    const result = await GetJobMetricsAction.run(
      {
        id: '1',
        userId: 'user-1',
        teamId: 'team-1',
  ...
  it('should call JobMetricsService on success', async () => {
    const mockData = { id: '1' };
  ...
  it('should return error when JobMetricsService fails', async () => {
    const serviceMock = vi.mocked(JobMetricsService);
    commonServiceMethods.forEach((m) => {
