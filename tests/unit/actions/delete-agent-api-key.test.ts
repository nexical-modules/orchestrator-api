...
vi.mock('../../../src/actions/../services/agent-api-key-service');

describe('DeleteAgentApiKeyAction', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('should be defined', () => {
    expect(DeleteAgentApiKeyAction).toBeDefined();
  });

  it('should run (smoke test)', async () => {
    const agentApiKeyServiceMock = vi.mocked(AgentApiKeyService);
    commonServiceMethods.forEach((m) => {
      if ((agentApiKeyServiceMock as unknown as Record<string, vi.Mock>)[m])
        (agentApiKeyServiceMock as unknown as Record<string, vi.Mock>)[m].mockResolvedValue({
          success: true,
          data: { id: '1' },
        });
    });
    ...
    const result = await DeleteAgentApiKeyAction.run(
      {
        id: '1',
        userId: 'user-1',
        teamId: 'team-1',
  ...
  it('should call AgentApiKeyService on success', async () => {
    const mockData = { id: '1' };
  ...
  it('should return error when AgentApiKeyService fails', async () => {
    const serviceMock = vi.mocked(AgentApiKeyService);
    commonServiceMethods.forEach((m) => {
...
import { AgentApiKeyService } from '../../../src/actions/../services/agent-api-key-service';
