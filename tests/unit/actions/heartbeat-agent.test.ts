...
vi.mock('../../../src/actions/../services/agent-service');
...
describe('HeartbeatAgentAction', () => {
  ...
  it('should be defined', () => {
    expect(HeartbeatAgentAction).toBeDefined();
  });

  it('should run (smoke test)', async () => {
    const agentServiceMock = vi.mocked(AgentService);
    commonServiceMethods.forEach((m) => {
      if ((agentServiceMock as unknown as Record<string, vi.Mock>)[m])
        (agentServiceMock as unknown as Record<string, vi.Mock>)[m].mockResolvedValue({
          success: true,
          data: { id: '1' },
        });
    });
    ...
    const result = await HeartbeatAgentAction.run(
      {
        id: '1',
        userId: 'user-1',
        teamId: 'team-1',
  ...
  it('should call AgentService on success', async () => {
    const mockData = { id: '1' };
  ...
  it('should return error when AgentService fails', async () => {
    const serviceMock = vi.mocked(AgentService);
    commonServiceMethods.forEach((m) => {
...
import { AgentService } from '../../../src/actions/../services/agent-service';
